...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new java.util.NoSuchElementException("no tweets in empty tweetset")
  
  def mostRetweetedAcc(acc: Tweet): Tweet = acc
  
  def getTweet: Tweet = throw new java.util.NoSuchElementException("no tweets in empty tweetset")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet = {
    val maxTweet = if (elem.retweets > acc.retweets) elem else acc
    if(left.mostRetweetedAcc(maxTweet).retweets > right.mostRetweetedAcc(maxTweet).retweets) left.mostRetweetedAcc(maxTweet) else right.mostRetweetedAcc(maxTweet)
  }
