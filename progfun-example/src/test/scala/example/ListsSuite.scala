...
class ListsSuite extends FunSuite {
  ...
  test("sum of a few numbers and a negative") {
    assert(sum(List(1,2,-5)) === -2)
  }
  
  test("sum of a only negative numbers") {
  ...
  try {
      ...
      case e: java.util.NoSuchElementException => // Expected
    }
  }
  
  test("max of strictly negative numbers") {
    assert(max(List(-3,-2,-1)) === -1)
  }
  
  test("max around the 0 boundary") {
